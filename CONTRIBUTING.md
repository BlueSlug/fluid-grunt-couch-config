...
acquainted with the community, review the [README file](README.md), and view the [source code](https://GitHub.com/fluid-project/fluid-grunt-couch-config).
...
The CouchConfig Grunt Task's [source code](https://github.com/fluid-project/fluid-grunt-couch-config) is hosted on GitHub. All of the code that is included in a release lives in the master branch.
...
The CouchConfig Grunt Task uses a workflow where contributors fork the project repository, work in a branch created off of master, and submit a pull request against the project repo's master branch to merge their contributions.
...
The CouchConfig Grunt Task uses a [JIRA](https://issues.fluidproject.org) issue tracker. All work should be associated with a
...
```bash
# Runs the CouchConfig Grunt Task's linting tasks
grunt lint
```
